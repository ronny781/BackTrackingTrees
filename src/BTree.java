...
public class BTree<T extends Comparable<T>> {
    ...
    //This list will store lists, each list will keep a record of an insertion info.
    protected Deque<Object> backTrackList = new LinkedList<Object>();
    ...
    public void insert(T value) {
        //This list will hold all the nodes that will be splitted and ultimately the node that the value will be inserted to
    	Deque<Object> list = new LinkedList<Object>();
        if (root == null) {
            root = new Node<T>(null, maxDegree);
            root.addKey(value);
            BacktrackingBTree.listElements element = new BacktrackingBTree.listElements(value,root);
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                	Node<T> splittedNode = currentNode;
                	...
                	//Need to insert the proper index of the left and right child
                    int idx = currentNode.getValuePosition(value);
                    int index = currentNode.getValuePosition(medianValue);
                    currentNode.getChild(index);//Left child of the splitted node
                    currentNode.getChild(index+1);//Right child of the splitted node
                    BacktrackingBTree.listElements element = new BacktrackingBTree.listElements(splittedNode,currentNode.getChild(index),currentNode.getChild(index+1));
                    list.addFirst(element);//Add to out list an element that hold the record of the splitted node(before splitted) and left and right children after the split
                ...
                if (currentNode.isLeaf()) {
                	...
                	wasAdded = true;
                	BacktrackingBTree.listElements element = new BacktrackingBTree.listElements(value,currentNode);
                	list.addFirst(element);//Save the inserted value node 
                } else {
                    int idx = currentNode.getValuePosition(value);
                    currentNode = currentNode.getChild(idx);
                }
        ...
        size++;
        backTrackList.addFirst(list);//Add to our BackTrack list this list of the last insertion
